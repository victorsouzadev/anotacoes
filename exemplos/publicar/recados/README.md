...
Um mural de recados: front estático (`web/`), API C# (`api/`) e SQLite. Serve de
...
| 3 | Banco em `ConnectionStrings:Default`. A plataforma aponta para `/data/app.db`, que **sobrevive a republicações**. O resto do disco é somente leitura | `builder.Configuration.GetConnectionString("Default")` |
