...
import { CATEGORY_COLORS, TaskProject } from '../models/task.model';
import { TasksStoreService } from '../services/tasks-store.service';

@Component({
  selector: 'app-tasks-projects-page',
  standalone: true,
  imports: [FormsModule, RouterLink, TasksTopBarComponent, IconComponent],
  template: `
    <div class="page">
      <app-tasks-top-bar />
      <main class="content">
        ...
        <div class="new-row">
          <input [(ngModel)]="newName" placeholder="Novo projeto" (keydown.enter)="create()" />
        ...
        @if (store.loading()) {
          ...
          <p class="empty">Nenhum projeto ainda. Crie um projeto e adicione as tarefas que participam dele.</p>
        } @else {
          <div class="list">
            @for (p of store.projects(); track p.id) {
              <div class="row">
                @if (editingId === p.id) {
                  <input class="name-input" [(ngModel)]="editingName" (keydown.enter)="commitRename(p)" (blur)="commitRename(p)" />
                  <div class="color-picker">
                    @for (color of colors; track color) {
                      <button
                        type="button"
                  ...
                  <a class="link" [routerLink]="['/tasks/projetos', p.id]">
                    <span class="dot" [style.background]="p.colorHex"></span>
                    <span class="name">{{ p.name }}</span>
                  </a>
                  <span class="count">{{ taskCount(p) }} tarefa(s)</span>
                }
                <div class="actions">
                  @if (editingId === p.id) {
                    <button (click)="commitRename(p)"><app-icon name="check" [size]="14" /></button>
                  } @else {
                    <button (click)="startEdit(p)"><app-icon name="edit" [size]="14" /></button>
                  }
                  <button class="danger" (click)="remove(p)"><app-icon name="delete" [size]="14" /></button>
                </div>
              </div>
            }
          </div>
  ...
  styles: [`
    ...
    .link { display: flex; align-items: center; gap: 10px; text-decoration: none; color: inherit; }
...
export class TasksProjectsPageComponent implements OnInit {
  ...
  taskCount(project: TaskProject): number {
    return this.store.membershipsFor(project.id).length;
  }

  async create(): Promise<void> {
    const name = this.newName.trim();
    if (!name) return;
    await this.store.createProject(name, this.newColor);
    this.newName = '';
    this.cdr.markForCheck();
  }
  ...
  startEdit(project: TaskProject): void {
    ...
    this.editingColor = project.colorHex;
  }

  async commitRename(project: TaskProject): Promise<void> {
    if (this.editingId !== project.id) return;
    this.editingId = null;
    const name = this.editingName.trim();
    if (!name) return;
    await this.store.renameProject(project, name, this.editingColor);
    this.cdr.markForCheck();
  }

  async remove(project: TaskProject): Promise<void> {
    if (!confirm(`Excluir o projeto "${project.name}"? O quadro e suas raias somem, mas as tarefas continuam existindo.`)) return;
