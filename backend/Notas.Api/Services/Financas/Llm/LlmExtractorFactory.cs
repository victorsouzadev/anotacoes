...
/// <summary>Provedor, chave e modelo já resolvidos — para quem precisa falar com
/// o LLM fora da extração de lançamentos (por exemplo, nomear imagens).</summary>
public record CredenciaisLlm(string Provedor, string? Chave, string Modelo);

public interface ILlmExtractorFactory
{
    Task<ConfiguracaoEfetiva> ResolverAsync(string userId, CancellationToken ct = default);

    Task<CredenciaisLlm> ResolverCredenciaisAsync(string userId, CancellationToken ct = default);
...
public class LlmExtractorFactory : ILlmExtractorFactory
{
    ...
    public async Task<CredenciaisLlm> ResolverCredenciaisAsync(string userId, CancellationToken ct = default)
    {
        var (provedor, chave, modelo, _) = await ResolverBrutoAsync(userId, ct);
        return new CredenciaisLlm(provedor, chave, modelo);
    }
